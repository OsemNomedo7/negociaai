...
export async function POST(req: NextRequest) {
  ...
  if (debtor) {
    ...
    debtData = {
      ...
      discount, score, scoreAfterPay,
      checkoutUrl: settings?.checkoutUrl || "",
    };
    await prisma.log.create({
      data: { name: name.trim(), cpf: formattedCPF, event: "CONSULTA", ip: geo.resolvedIp, city: geo.city, state: geo.state, debtorId: debtor.id },
    });
  } else {
    ...
    debtData = {
      ...
      discountedAmount: Math.round(discountedAmount * 100) / 100,
      description: defaultDesc, status: "PENDENTE",
      discount, score, scoreAfterPay,
      checkoutUrl: settings?.checkoutUrl || "",
    };
    await prisma.log.create({
      data: { name: name.trim(), cpf: formattedCPF, event: "CONSULTA", ip: geo.resolvedIp, city: geo.city, state: geo.state },
    });
  }
