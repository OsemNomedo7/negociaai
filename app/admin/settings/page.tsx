...
type TabKey = "empresa" | "cores" | "banners" | "copy-hero" | "copy-consulta" | "copy-sobre" | "copy-contato" | "negocio" | "webhook";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/settings")
      ...
      .then(d => {
        if (!d) return;
        setSettings({ ...DEFAULT_SETTINGS, ...d, bannerImages: parseBannerImages(d.bannerImages), logoHeight: d.logoHeight ?? 44, pageTitle: d.pageTitle ?? d.companyName ?? "NegociAI" });
  ...
  function updS<K extends keyof Settings>(k: K, v: Settings[K]) { setSettings(s => ({ ...s, [k]: v })); }
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-4xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black" style={{ color: "var(--text)" }}>Configurações</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--text-muted)" }}>Personalize todo o sistema</p>
        </div>
            ...
            <div className="grid md:grid-cols-2 gap-4 items-start">
              <div className="space-y-3">
                ...
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                  Ou use a URL: <code
                    className="px-1.5 py-0.5 rounded text-xs cursor-pointer font-mono"
              ...
              <div key={b.label} className="mb-5 p-4 rounded-xl" style={{ background: "rgba(99,102,241,.04)", border: "1px solid var(--border)" }}>
                ...
                <div className="grid md:grid-cols-2 gap-4 items-start">
                  <ImageUpload
                    ...
                    value={pageContent[b.urlKey] as string}
                    onChange={v => updPC(b.urlKey, v)}
                    hint="PNG, JPG ou SVG · proporção recomendada 6:1"
                  />
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: "var(--text)" }}>Link ao clicar</label>
          ...
          <div className="flex items-start gap-2 p-3 rounded-xl" style={{ background: pageContent.promoBannerUrl ? "rgba(249,115,22,.07)" : "rgba(99,102,241,.06)", border: `1px solid ${pageContent.promoBannerUrl ? "rgba(249,115,22,.3)" : "rgba(99,102,241,.2)"}` }}>
            ...
            <p className="text-xs" style={{ color: pageContent.promoBannerUrl ? "#ea580c" : "var(--text-muted)" }}>
              {pageContent.promoBannerUrl
                ? <><strong>Banner estático ativo.</strong> Estas copys não estão sendo exibidas no site — o banner substitui toda a área do hero. Para usar estas copys, remova o banner na aba <strong>Banners</strong>.</>
        ...
        {activeTab === "webhook" && (<>
          ...
          <div className="p-4 rounded-xl text-sm" style={{ background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.2)" }}>
            ...
            <p style={{ color: "var(--text-muted)" }}>
              ...
              e registra o evento <code className="px-1 py-0.5 rounded text-xs" style={{ background: "var(--border)" }}>PAGAMENTO_CONCLUIDO</code> nos logs.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs mb-2" style={{ color: "var(--text-muted)" }}>
              Configure esta URL no painel do seu processador de pagamento. O parâmetro <code className="px-1 rounded text-xs" style={{ background: "var(--border)" }}>?secret=</code> é o token de segurança definido abaixo.
            </p>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={e => updS("webhookSecret", e.target.value)}
                className="input-field font-mono flex-1"
                placeholder="ex: wh_k9x2mLp3nQ7rT1vZ..."
              />
              <button
                type="button"
                onClick={() => {
                  const chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
                  const secret = "wh_" + Array.from({ length: 32 }, () => chars[Math.floor(Math.random() * chars.length)]).join("");
                  updS("webhookSecret", secret);
                }}
